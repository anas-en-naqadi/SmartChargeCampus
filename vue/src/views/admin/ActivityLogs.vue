    ...
    <div class="card mt-12 m-6 bg-white border border-gray-300 rounded-xl">
        ...
        <div v-if="!loading">
            <DataTable :value="filteredActivities" removableSort :paginator="true" class="p-datatable-sites -mt-5 m-6 "
                ...
                <template #header>
                    <div class="flex justify-between items-center gap-5" style="display: flex">
                        <span class="relative">
                            ...
                            <InputText @input="filterTable($event)" placeholder="Recherche" id="searchInput"
                                class="pl-10 py-2 border-gray-300" />
                        </span>
                        <Button type="button" icon="pi pi-filter-slash" label="Effacer"
                ...
                <!-- Columns -->
                <Column field="" header="Nom d'utilisateur" class="border-b-[1px] text-center">
                    <template #body="{ data }">
                        <span>{{ data.causer.name }}</span>
                    </template>
                </Column>
                <Column field="description" header="Événement" class="border-b-[1px] text-center">
                </Column>
                ...
                <Column field="created_at" header="Date de l'opération" sortable class="border-b-[1px] text-center">
                    <template #body="{ data }">
                ...
                <template #header>
                    <div class="flex justify-between items-center gap-5" style="display: flex">
                        <span class="relative">
                            <i
                                class="pi pi-search absolute top-2/4 -mt-2 left-3 text-surface-400 white:text-surface-600" />
                            <InputText :disabled="loading" placeholder="Recherche" class="pl-10 py-2" />
                        </span>
                        <Button type="button" icon="pi pi-filter-slash" label="Effacer"
                            class="p-button-outlined bg-white py-2 px-4 border border-blue-500 rounded-md text-blue-800 hover:text-white hover:bg-blue-500"
                            @click="clearFilter()" />
