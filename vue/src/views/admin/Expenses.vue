<template>
    <div class="card mt-12 m-6 bg-white border border-gray-300 rounded-xl">
        <div class="flex justify-between items-center mb-4">
            ...
            <Button class="text-sm text-white -mt-5 font-bold py-2 px-4 ml-6 rounded-xl bg-green-600 hover:bg-green-700"
                @click="visible = true">
                إضافة مصروف
            </Button>
            <p class="mb-4 text-2xl flex font-extrabold p-8">
                <span class="xl:text-2xl text-xl font-bold text-red-800">-{{ common.formatNumber(total) }} DH</span>
                <span class="ml-1"> : مجموع المصاريف</span>
            </p>
        </div>
        <!-- Main Data Table -->
        <div v-if="!loading">
            <DataTable :value="filteredExpenses" removableSort :paginator="true" class="p-datatable-sites -mt-5 m-6"
                ...
                <!-- Columns -->
                <Column field="name" header="إسم الرسم" class="border-b-[1px] text-center ">

                </Column>
                <Column sortable field="amount" header="المبلغ" class="border-b-[1px] text-center">
                    <template #body="{ data }">
                        <span> {{ common.formatNumber(data.amount) }} درهم</span>
                    </template>
                </Column>
                <Column field="created_at" header="تمت إضافته في" class="border-b-[1px] text-center " sortable>
                    <template #body="{ data }">
                ...
                <Column header="إجراءات" class="border-b-[1px] text-center">
                    <template #body="{ data }">
                        <div class="flex gap-3">
                            ...
                            <button title="Delete this product" @click="deleteExpense(data.id)">
        ...
        <div v-else>
            <DataTable :value="skeletonObjects" class="p-datatable-sites -mt-5 m-6" tableStyle="min-width: 30rem"
                ...
                <Column field="" header="إسم الرسم" style="min-width: 12rem"
                ...
                <Column field="" header=" المبلغ" class="text-center border-b-2">
                    <template #body>
                        <Skeleton></Skeleton>
                    </template>
                </Column>
                <Column field="" header="تمت إضافته في" class="text-center border-b-2">
                    <template #body>
                        <Skeleton></Skeleton>
                    </template>
                </Column>
                <Column field="" header="إجراءات" class="text-center border-b-2">
                    <template #body>
                        <Skeleton></Skeleton>
                    </template>
                </Column>
        ...
        <Dialog v-model:visible="visible" modal header="إضافة مصروف" class="w-[50%]">
            <div class="w-full">
                <div class="relative z-0 w-full mb-5 group">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-500">إسم الرسم</label>
                    <input type="text" id="name" v-model="expense.name"
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="amount" class="block mb-2 text-sm font-medium text-gray-500">المبلغ</label>
                    <input type="text" id="amount" v-model="expense.amount"
                        class="block py-2 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="0.0000" />
...
function addExpense() {
    store.dispatch("storeExpense", expense.value).then((res) => {
...
function filterTable(event) {
    ...
    if (!filter) filteredExpenses.value = products.value;
    else
